...
public class User extends Thread {
	...
	public void run() {
		while (true) {
			...
			try {
				...
				boolean isSuccess = attack(attackerId, damage, targetGalaxy.id, targetConnection);
				callableStatement.close();
				
				if (isSuccess) {
					sql = "{CALL LOG(?, ?)}";
					String logText = String.format("User %d attacked %s! %s damaged %d!", attackerId, targetGalaxy.name, targetGalaxy.name, damage);
					callableStatement = connection.prepareCall(sql);
	...
	private boolean attack(int attackerId, int damage, int targetId, Connection targetConnection) throws SQLException {
